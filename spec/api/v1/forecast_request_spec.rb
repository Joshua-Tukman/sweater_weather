...
RSpec.describe 'Forecast request' do
  it "Sends a current 7 day forecast with hourly current temp, high/lows temps/, pressure, 'feels like' temp, humidity, visibility, UV index, sunrise/sunset times" do
    VCR.use_cassette("get_forecast_good_info") do
      ...
      expect(json[:data][:attributes][:forecast][:time]).to eq("1:56 PM, September 20")
      expect(json[:data][:attributes][:forecast][:current_temp]).to eq(65)
      expect(json[:data][:attributes][:forecast][:feels_like]).to eq(64)
      expect(json[:data][:attributes][:forecast][:humidity]).to eq(93)
      expect(json[:data][:attributes][:forecast][:visibility]).to eq(10000)
      expect(json[:data][:attributes][:forecast][:uv_index]).to eq(9)
      expect(json[:data][:attributes][:forecast][:sunrise]).to eq("5:33 AM")
      expect(json[:data][:attributes][:forecast][:sunset]).to eq("5:45 PM")
      expect(json[:data][:attributes][:forecast][:high_temp]).to eq(69)
      expect(json[:data][:attributes][:forecast][:low_temp]).to eq(64)
      #test hourly info
      ...
      expect(json[:data][:attributes][:forecast][:hourly].first[:temp]).to eq(65)
      expect(json[:data][:attributes][:forecast][:hourly].first[:time]).to eq("1 PM")
      #test daily info 
      expect(json[:data][:attributes][:forecast][:five_day_forecast].count).to eq(5)
      expect(json[:data][:attributes][:forecast][:five_day_forecast].first[:day]).to eq("Monday")
      ...
      expect(json[:data][:attributes][:forecast][:five_day_forecast].first[:high_temp]).to eq(77)
      expect(json[:data][:attributes][:forecast][:five_day_forecast].first[:low_temp]).to eq(68)
    end          
  end
